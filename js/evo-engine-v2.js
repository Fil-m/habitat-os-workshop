...
class PhysicsEngine {
  ...
  stop() {
    ...
    window.worldPanX = 0; window.worldPanY = 0; window.worldPanZ = 0;
  ...
  loop() {
    ...
    // Movement based on camera angle
    const speed = 5;
    if (window.inputState && window.getCameraRotY) {
      let forwardX = 0, forwardZ = 0;
      let rightX = 0, rightZ = 0;
      const angle = window.getCameraRotY() * Math.PI / 180;
      ...
      if(window.inputState.up) { this.player.vx += forwardX * speed * dt; this.player.vy -= forwardZ * speed * dt; }
      if(window.inputState.down) { this.player.vx -= forwardX * speed * dt; this.player.vy += forwardZ * speed * dt; }
      if(window.inputState.left) { this.player.vx -= rightX * speed * dt; this.player.vy += rightZ * speed * dt; }
      if(window.inputState.right) { this.player.vx += rightX * speed * dt; this.player.vy -= rightZ * speed * dt; }
    ...
    // Camera follow (center scene on player)
    window.worldPanX = -posX;
    window.worldPanY = -posY;
    window.worldPanZ = -posZ;
    if(window.updateCamera) window.updateCamera();
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  sceneEl.addEventListener('touchmove', (e) => {
    ...
    if(activeTool !== 'select') return; // Prevent accidental multiple cubes; only allow drag for selection on mobile
  ...
  function handleInteraction(e) {
    ...
    if (target.classList.contains('face')) {
      ...
      if (e.type === 'touch') return; // Touch relies on faces mostly
  ...
  window.getCameraRotY = () => rotY;

  window.updateCamera = () => {
    const worldPan = `translate3d(${window.worldPanX}px, ${window.worldPanY}px, ${window.worldPanZ}px)`;
    if (renderer.viewMode === '3d') {
      sceneEl.style.transform = `scale(${scale}) translate3d(${panX}px, ${panY}px, 0) rotateX(${rotX}deg) rotateY(${rotY}deg) ${worldPan}`;
    } else if (renderer.viewMode === 'top') {
      sceneEl.style.transform = `scale(${scale}) translate3d(${panX}px, ${panY}px, 0) rotateX(-90deg) rotateY(0deg) rotateZ(0deg) ${worldPan}`;
    } else if (renderer.viewMode === 'side') {
      sceneEl.style.transform = `scale(${scale}) translate3d(${panX}px, ${panY}px, 0) rotateX(0deg) rotateY(0deg) rotateZ(0deg) ${worldPan}`;
    }
  };
  ...
  document.getElementById('viewport').addEventListener('touchstart', (e) => {
    if(e.touches.length === 2) {
      ...
      if(activeTool === 'camera' || (!e.target.classList.contains('face') && !e.target.classList.contains('grid-floor'))) {
        camDragging = true;
